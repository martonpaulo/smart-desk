...
export function createSyncedEntityStore<E extends Base>(cfg: SyncedEntityStoreConfig<E>) {
  ...
  const store = createZustandEntityStore<E>({
    ...
    upsert: (_client, item) => service.upsert(item),
    softDelete: (_client, id) => service.softDelete(id),
    restore: (_client, id) => service.restore(id),
    hardDelete: (_client, id) => service.hardDelete(id),
